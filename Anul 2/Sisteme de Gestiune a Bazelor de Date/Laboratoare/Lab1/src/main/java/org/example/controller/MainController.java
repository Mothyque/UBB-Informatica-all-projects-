...
public class MainController extends AbstractController
{
    ...
    private Service<Integer, Class> classService;
    ...
    private final ObservableList<Class> classesModel = FXCollections.observableArrayList();
     ...
     */
    public void setServices(Service<Integer, Student> studentService, Service<Integer, Professor> professorService, Service<Integer, Class> classService)
    {
    ...
    public void initData()
    {
        ...
        {
            TableColumn<Class, String> nameColClass = new TableColumn<>("Name");
            nameColClass.setCellValueFactory(new PropertyValueFactory<>("name"));

            TableColumn<Class, Integer> creditsColClass = new TableColumn<>("Credits");
    ...
    public void onAddClass()
    {
        try
        {
            ...
            Class cls = new Class(name, Integer.parseInt(creditsText), selectedProfessor.getId());
            classService.add(cls);
            showInfoMessage("Class added successfully.");
        }
        catch(ValidationException e)
        {
            showErrorMessage(e.getMessage());
    ...
    public void onDeleteClass()
    {
        try
        {
            Class selected = tblClasses.getSelectionModel().getSelectedItem();
            if(selected != null)
            {
                Optional<ButtonType> result = confirmDeletion();
                if(result.isPresent() && result.get() == ButtonType.OK)
                {
                    ...
                    showInfoMessage("Class deleted successfully.");
                }
            }
            else
            {
    ...
    public void onUpdateClass()
    {
        try
        {
            ...
            if(selected != null)
            {
                ...
                selected.setProfessorId(selectedProfessor.getId());
                classService.update(selected);
                showInfoMessage("Class updated successfully.");
            }
            else
            {
    ...
    private void refreshClasses()
    {
        ...
        if(selectedProfessor != null)
        {
            ...
            classes.forEach(cls -> {
                if(cls.getProfessorId().equals(selectedProfessor.getId()))
                {
    ...
    /**
     * Validează regulile de business pentru entitatea Class (ex: credite între 0 și 30).
     * @throws ValidationException dacă datele sunt invalide.
     */
