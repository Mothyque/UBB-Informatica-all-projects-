        ...
        try
        {
            ...
            try
            {
                ...
                if ("GET".equalsIgnoreCase(method) && path.matches("/api/jocuri/[a-zA-Z0-9]+"))
                {
                    ...
                    try
                    {
                        List<Joc> jocuri = service.getJocuriCuHit(alias);
                ...
                if ("POST".equalsIgnoreCase(method) && path.matches("/api/barci") )
                {
                    Map<String, Object> body = objectMapper.readValue(exchange.getRequestBody(), Map.class);
                    String pozitii = body.get("pozitii").toString();
                    service.adaugaBarca(pozitii);
